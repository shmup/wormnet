...
def test_request_unknown_channel_scheme(client):
    ...
    response = client.get('/wormageddonweb/RequestChannelScheme.asp?Channel=nonexistent')
    assert response.status_code == 200
    text = response.get_data(as_text=True)
...
def test_game_create(client):
    ...
    """
    response = client.get('/wormageddonweb/Game.asp', query_string={
        "Cmd": "Create",
        "Name": "Test Game",
        "Nick": "TestPlayer",
    ...
    # should have SetGameId header
    assert "SetGameId" in response.headers, f"Missing SetGameId header: {response.headers}"

    # extract game id from header
    header_value = response.headers["SetGameId"]
    import re
    match = re.search(r':\s*(\d+)', header_value)
    assert match, f"Can't parse game ID from header: {header_value}"
...
def test_game_list(client):
    ...
    """
    # create a game first
    client.get('/wormageddonweb/Game.asp', query_string={
        "Cmd": "Create",
        "Name": "Listed Game",
        "Nick": "Host1",
        "HostIP": "1.2.3.4:17011",
